...
public class ActorBeta extends Actor {
    ...
    private Vector2 simpleVelocityVec;
    ...
    public ActorBeta() {
        ...
        boundaryCircle = null;
        simpleVelocityVec = new Vector2(0, 0);
    }
     ...
     */
    // set velocity based on destination coordinates
    public void SetVelocity(float destX, float destY)
    {
        float velX = destX - this.getX();
        float velY = destY - this.getY();
        float len = Vector2.dst(this.getX(), this.getY(), destX, destY);
        simpleVelocityVec.set(5.0f * velX / len, 5.0f * velY / len);
    }
    // set velocity based on its x and y components
    public void SetVelocityXY(float x, float y)
    {
        simpleVelocityVec.set(x, y);
    }

    public Vector2 GetVelocity (){return simpleVelocityVec;}
    /**
     *  //PHYSICS METHODS
     * @param width
     * @param height
    ...
    public void setBoundaryCircle()
    {
        float x = getWidth() / 2;
        float y = getHeight() / 2;
        float radius = getWidth() /4;
        boundaryCircle = new Circle(x, y, radius);
    }
